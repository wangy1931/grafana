...
import TableModel from '../../../core/cw_table_model';
...
transformers['json'] = {
  ...
  transform: function(data, panel, model) {
    var i, y, z;
    // copy all columns, not only column.text
    for (i = 0; i < panel.columns.length; i++) {
      // model.columns.push({text: panel.columns[i].text});
      model.columns.push(_.cloneDeep(panel.columns[i]));
    }

    if (model.columns.length === 0) {
      model.columns.push({ text: '_source', value: '_source' });
    }
