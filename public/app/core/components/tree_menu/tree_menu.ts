...
export class TreeMenuCtrl {
  ...
  loadAssociatedMetrics(params) {
    this.alertMgrSrv.loadAssociatedMetrics(params).then(
      res => {
  ...
  checkSource(metric, host) {
    return (
      _.isEqual(metric, this.associationSrv.sourceAssociation.metric) &&
  ...
  initByTime() {
    ...
    if (diff > this.limitTime) {
      this.$scope.appEvent('alert-warning', [
        '您仅可以关联' + this.limitTime + '天以内的数据',
        '请选择' + this.limitTime + '天以内的时间区间',
      ]);
      return;
    }
