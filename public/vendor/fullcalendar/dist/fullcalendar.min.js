...
var DayTableMixin = FC.DayTableMixin = {
	...
	sliceRangeByRow: function(range) {
		...
		var rangeLast = this.getDateDayIndex(range.end); // inclusive last index
		// cloudwiz end
		var segs = [];
		var row;
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		// cloudwiz start
		var left = (seg.isStart ? moment(seg.eventStartMS).subtract(8, 'hours').hours() / 24 / (seg.rightCol - seg.leftCol + 1) * 100 : 0 );
		var right = (seg.isEnd ? (24 - moment(seg.eventStartMS + seg.eventDurationMS).subtract(8, 'hours').hours()) / 24 / (seg.rightCol - seg.leftCol + 1) * 100 : 0);
		skinCss += ';left:' + left + '%;';
		skinCss += 'right:' + right + '%;';
		// cloudwiz end 
		var timeHtml = '';
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<div class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
			...
			html +=
				'<td data-time="' + slotDate.format('HH:mm:ss') + '"' +
					(isLabeled ? '' : ' class="fc-minor"') +
					'>' +
					(!isRTL ? axisHtml : '') +
					'<div class="' + view.widgetContentClass + '"/>' +
					(isRTL ? axisHtml : '') +
				"</td>";
...
TimeGrid.mixin({
	...
	generateFgSegHorizontalCss: function(seg) {
		...
		// cloudwiz update start
		var range = Math.round(seg.end.diff(seg.start, 'minutes') / 60);
		left = seg.start.hours();
		var tmp = left + range;
		right = tmp > 24 ? 0 : (24 - tmp);

		props.zIndex = seg.level + 1; // convert from 0-base to 1-based
		props.left = left * 100 / 24 + '%';
		props.right = right * 100 / 24  + '%';
