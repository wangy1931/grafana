...
import (
	...
	"github.com/wangy1931/grafana/pkg/components/dashdiffs"
	...
	"github.com/wangy1931/grafana/pkg/services/alerting"
...
func GetDashboardVersions(c *middleware.Context) Response {
	...
	for _, version := range query.Result {
		if version.RestoredFrom == version.Version {
			version.Message = "初始化 (created by migration)"
			continue
		}

		if version.RestoredFrom > 0 {
			version.Message = fmt.Sprintf("恢复来自版本 %d", version.RestoredFrom)
			continue
		}
...
func GetAlertDef(c *middleware.Context) {
	var dat map[string]interface{}
	name := c.Params(":name")
	filePath := path.Join(setting.StaticRootPath, "dashboards/alertd/" + name + ".json")
	file, err := ioutil.ReadFile(filePath)
	if err != nil {
		c.JsonApiErr(500, "File isn't exist", err)
		return
...
func GetMetricHelpFile(c *middleware.Context) {
	var dat map[string]interface{}
	name := c.Params(":name")
	filePath := path.Join(setting.StaticRootPath, "dashboards/metric/" + name + ".json")
	file, err := ioutil.ReadFile(filePath)
	if err != nil {
		c.JsonApiErr(500, "File isn't exist", err)
		return
